...
function returnGoods(): void {
  ...
  inquirer.prompt([
    ...
    },
    {
      ...
      validate: (value) => !isNaN(Number(value)) && Number(value) > 0 ? true : "Enter a valid positive number",
      filter: (input) => parseFloat(input),
    },
    {
      type: "input",
  ...
  ]).then(({ clientOrMerchantId, object, name, description, material, weight, price, reason }) => {
    const entity = clientCollection.getClientById(clientOrMerchantId) || merchantCollection.getMerchantById(clientOrMerchantId);
    ...
    switch (object) {
      case "potion":
        newItem = new Potion(newId, name, description, material, weight, price, "None");
        break;
      case "armor":
        newItem = new Armor(newId, name, description, material, weight, price);
        break;
      case "weapon":
        newItem = new Weapon(newId, name, description, material, weight, price);
        break;
      default:
        return showError("Invalid item type");
    }
